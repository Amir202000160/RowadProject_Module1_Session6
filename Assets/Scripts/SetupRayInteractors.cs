...
public class SetupRayInteractors : MonoBehaviour // Inherit from MonoBehaviour to make this a Unity component
{
    ...
    [SerializeField] private Material rayMaterial; // Material to make the ray visible (color, texture, etc.)
    ...
    void Start()
    {
        ...
        if (!leftController.TryGetComponent<XRRayInteractor>(out var _))
        {
            ...
            // Start with the ray disabled (user can enable it later with input)
